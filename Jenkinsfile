		  ...
		  stage('Deploy to Minikube') {
            steps {
                ...
                withCredentials([string(credentialsId: 'secrets', variable: KUBE_SA_TOKEN)]) {
                    bat 'kubectl apply -f C:\\Desenvolvimento\\vscode-workspace\\ApiRestMessage\\deployment.yaml --token=$KUBE_SA_TOKEN --server=https://127.0.0.1:54840 --insecure-skip-tls-verify=true --validate=false'
